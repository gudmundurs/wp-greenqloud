...
class GreenQloudWordPressPlugin extends GreenQloudWordPressPluginPublic {
    
    function GreenQloudWordPressPlugin() {
		parent::GreenQloudWordPressPluginPublic();
        ...
        if (!$this->options['hideAmazonS3UploadTab']) {
			...
			add_action('media_upload_tantan-wp-greenqloud', array(&$this, 'media_upload_content'));
		}
        add_action('activate_tantan/wp-greenqloud.php', array(&$this, 'activate'));
        if ($_GET['tantanActivate'] == 'wp-greenqloud') {
    ...
    // this should install the javascripts onto the user's s.greenqloud.com account
    ...
    function activate() {
        wp_redirect('plugins.php?tantanActivate=wp-greenqloud');
        exit;
    }
    function deactivate() {}
    
    function showConfigNotice() {
        add_action('admin_notices', create_function('', 'echo \'<div id="message" class="updated fade"><p>GreenQloud Plugin for WordPress <strong>activated</strong>. <a href="options-general.php?page=tantan/wp-greenqloud/class-plugin.php">Configure the plugin &gt;</a></p></div>\';'));
    }

	function settings() {
		add_options_page('GreenQloud', 'GreenQloud', 10, __FILE__, array(&$this, 'admin'));
		$this->version_check();
	}
    function addhooks() {
		parent::addhooks();
        if (!$_POST['disable_greenqloudS3']) {
    ...
    function version_check() {
        ...
        if (is_object($TanTanVersionCheck)) {
            $data = get_plugin_data(dirname(__FILE__).'/../wp-greenqloud.php');
    ...
    function admin() {
        if ($_POST['action'] == 'save') {
            ...
            $options = get_option('greenqloud_wordpress_s3');
            ...
            update_option('greenqloud_wordpress_s3', $_POST['options']);
            
            if ($_POST['options']['bucket']) {
                $options = get_option('greenqloud_wordpress_s3');
                require_once(dirname(__FILE__).'/lib.s3.php');
                $s3 = new GreenQloudS3($options['key'], $options['secret']);
                ...
                $message = "Saved GreenQloud authentication information. ";
            }
            if (function_exists('dns_get_record') && $_POST['options']['virtual-host']) {
                $record = dns_get_record($_POST['options']['bucket']);
                if (($record[0]['type'] != 'CNAME') || ($record[0]['target'] != $_POST['options']['bucket'].'s.greenqloud.com')) {
  ...
  //Delete the file(s) on S3
    function delete_attachment($postid) {
		...
		if (is_array($this->meta) && $this->meta["postID"]==$postid) {
			...
			require_once(dirname(__FILE__).'/lib.s3.php');
 	        $this->s3 = new GreenQloudS3($this->options['key'], $this->options['secret']);
	...
	function wp_delete_file($file) {
 	...
 	function wp_get_attachment_metadata($data=false, $postID=false) {
		...
		if (is_numeric($postID)) {
			$this->meta = get_post_meta($postID, 'greenqloudS3_info', true);
    ...
    */
		...
		if (file_exists($data['file'])) {
			...
			if ($this->s3->putObjectStream($this->options['bucket'], $prefix.$file['name'], $file)) {
			  $keys = array($prefix.$file['name']);  
			    ...
			    delete_post_meta($postID, 'greenqloudS3_info');
                add_post_meta($postID, 'greenqloudS3_info', array(
	...
	function media_buttons($context) {
		...
		$image_title = 'GreenQloud';
		
		$uploading_iframe_ID = (int) (0 == $post_ID ? $temp_ID : $post_ID);

		$media_upload_iframe_src = "media-upload.php?post_id=$uploading_iframe_ID";
		$out = ' <a href="'.$media_upload_iframe_src.'&tab=tantan-wp-greenqloud&TB_iframe=true&height=500&width=640" class="thickbox" title="'.$image_title.'"><img src="'.$image_btn.'" alt="'.$image_title.'" /></a>';
		return $context.$out;
	}
	...
	*/
        ...
        if ($this->options['key'] && $this->options['secret'] && $this->options['bucket']) {
            ...
            $tab = array(
                'tantan_amazons3' => array('GreenQloud', 'upload_files', array(&$this, 'tab'), $paged, $args),
